...
function Home({ favoriteList }) {
  ...
  const handleSearch = useCallback(
    debounce(
      async (searchQuery, recurrency, skipAmount = 0, oldProducts = []) => {
  ...
  async function getSearchProducts(
    ...
    const response = await api.post(
      `/product/search?take=${take}&skip=${skipAmount}`,
      {
        search: searchQuery,
      }
    );
  ...
  async function getProducts(reset = false) {
    ...
    const response = await api.get(
      `/product/pagination?take=${take}&skip=${skip}`
    );
    setTotal(response.data.productsCount);
